...
void ic_rescaleImage(struct Image* image, struct vector dimensions){
    uint8_t *newPixels = 
        malloc(dimensions.x * dimensions.y * sizeof(uint8_t) * 4);
    for (int32_t x = 0; x < dimensions.x; x++){
        for (int32_t y = 0; y < dimensions.y; y++){
            uint32_t transformedX = 
                (x / (float) dimensions.x) * image->size.x;
            uint32_t transformedY = 
                (y / (float) dimensions.y) * image->size.y;
